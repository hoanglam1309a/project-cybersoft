...
export const bookingTicketReducer = (state = initialState, action) => {
  let cloneDataSelectingSeat = [...state.dataSelectingSeat];
  switch (action.type) {
    case "bookingTicket/selectSeat":
      //check selecting seat payload is has in state or no has, if no has return -1
      const isExistSeat = cloneDataSelectingSeat.findIndex((item) => {
        return item.soGhe === action.payload.soGhe;
      });
      if (isExistSeat === -1) {
        cloneDataSelectingSeat.push(action.payload);
      } else {
        cloneDataSelectingSeat.splice(isExistSeat, 1);
      }
      return { ...state, dataSelectingSeat: cloneDataSelectingSeat };

    case "bookingTicket/unselectSeat":
      const getIndexSeatRemove = cloneDataSelectingSeat.findIndex((item) => {
        return item.soGhe === action.payload.soGhe;
      });
      cloneDataSelectingSeat.splice(getIndexSeatRemove, 1);
      return { ...state, dataSelectingSeat: cloneDataSelectingSeat };
